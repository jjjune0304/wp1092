  ...
  const handleQuery = async () => {
    const {
      data: { messages, message },
         ...
         params : { queryType: queryType, queryString: queryString} 
      }); // TODO: axios.xxx call the right api
    console.log(message);
    console.log(messages);
    if (!messages) addErrorMessage(message);
    else {
      let sorted_messages = messages.slice().sort((a, b) => b.score - a.score);
      sorted_messages = sorted_messages.map((m) => m.msg); 
      addRegularMessage(...sorted_messages);
    }
  };
